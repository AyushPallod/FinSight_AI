...
def process_document_task(document_id: int, file_path: str, filename: str) -> None:
    ...
    """
    logger.info(f"Background task picked up: processing document '{filename}' (ID: {document_id})")
    ...
    try:
        ...
        if not doc:
            logger.error(f"Failed task: Document ID {document_id} was not found in SQLite.")
            return
        ...
        logger.info(f"Vector-indexing {len(chunks)} chunks in Qdrant for document ID {document_id}...")
        vector_store_service.upsert_document_chunks(chunks)

        # 6. Mark status as 'completed'
        doc.upload_status = "completed"
        db.commit()
        logger.info(f"Background task succeeded! Document ID {document_id} marked as 'completed'.")

    except Exception as e:
        logger.exception(f"Background task failed for document ID {document_id} due to: {str(e)}")
        ...
        if os.path.exists(file_path):
            try:
                ...
                logger.warning(f"Could not remove temp file {file_path}: {str(cleanup_err)}")
