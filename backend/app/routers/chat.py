...
async def chat_interaction(
    ...
    try:
        ...
        is_grounding_reject = (
            "do not contain the information required" in answer.lower()
        )
        ...
        if not is_grounding_reject:
            ...
            has_explicit_citations = any(
                (
                    c["source_filename"].lower() in answer.lower()
                    or os.path.splitext(c["source_filename"])[0].lower()
                    in answer.lower()
                )
                for c in chunks
            )
            ...
            for c in chunks:
                ...
                if has_explicit_citations:
                    ...
                    is_cited = (
                        filename.lower() in answer.lower()
                        or os.path.splitext(filename)[0].lower() in answer.lower()
                    )
                    if not is_cited:
                        continue
                ...
                if citation_key not in seen_citations:
                    ...
                    citations.append(
                        CitationItem(document=filename, page=page, snippet=snippet)
                    )
